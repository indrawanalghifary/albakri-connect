...
export function ChatInterface() {
  ...
  React.useEffect(() => {
    ...
    // If we switch to mobile, a chat might remain selected.
    // If we switch back to desktop and no chat is selected, select one.
  ...
  const ConversationList = () => (
    <Card className={cn(
        "h-full flex-col",
        // On mobile, hide if a conversation is selected. Full width if shown.
        isMobile ? (selectedConversation ? 'hidden' : 'flex w-full') : 'flex w-1/3',
    )}>
  ...
  const ChatWindow = ({ conversation }: { conversation: Conversation }) => {
    ...
    return (
        <Card className={cn(
            "h-full flex-col",
            // On mobile, this component is only rendered when a convo is selected, so it's always flex
            // On desktop, it takes up the remaining space
            isMobile ? 'flex w-full' : 'flex flex-1'
        )}>
  ...
  const EmptyChatState = () => (
     <div className="h-full hidden flex-1 flex-col items-center justify-center text-muted-foreground md:flex">
  ...
  return (
    <div className="flex h-full w-full justify-center gap-4">
        <ConversationList />
        {selectedConversation ? <ChatWindow conversation={selectedConversation} /> : <EmptyChatState />}
    </div>
  );
}
